...
# ===== NUEVA IMPORTACIÓN =====
from modules.traffic_controller import TrafficAction
...
class WAFModule(Module):
    async def run(self):
        ...
        # ===== NUEVO: Obtener referencia al Traffic Controller =====
        ...
        if hasattr(self, '_orchestrator') and self._orchestrator:
            if 'traffic_controller' in self._orchestrator.modules:
                ...
                await self.log('info', '🔗 WAF conectado al Traffic Controller')
            else:
        ...
        enable_cmd_injection = self.config.get('enable_cmd_injection', False)
        ...
        async def handle(request):
            ...
            if self.is_malicious(normalized_path, body):
                ...
                await self.alert('waf_block', f'Petición maliciosa bloqueada desde {client_ip}: {path}', {
                    'path': path,
                    'method': request.method,
                    'client_ip': client_ip
                })
    ...
    async def _proxy_request(self, request, backend):
